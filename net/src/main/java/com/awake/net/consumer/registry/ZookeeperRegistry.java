...
public class ZookeeperRegistry implements IRegistry {
     ...
     */
    private final Set<RegisterVO> providerHashConsumerSet = new CopyOnWriteArraySet();
    /**
     * addListener中的cache全部会被添加到这个集合中，这个集合不包括providerCuratorCache
     */
    private final List<CuratorCache> listenerList = new CopyOnWriteArrayList<>();
    ...
    private void doCheckConsumer() {
        ...
        for (var providerCache : providerHashConsumerSet) {
            ...
            // getClientSessionMap
            // 遍历自身已经连接上的provider
            var providerClientList = new ArrayList<Session>();
            NetContext.getSessionManager().forEachClientSession(new Consumer<Session>() {
                @Override
                public void accept(Session session) {
                    if (session.getProviderAttribute() != null && session.getProviderAttribute().getId().equals(providerCache.getId())) {
                        providerClientList.add(session);
                    }
                }
            });

            if (providerClientList.size() == 1) {
                var provider = providerClientList.get(0);
                if (SessionUtils.isActive(provider)) {
                    continue;
                } else {
                    recheckFlag = true;
                    NetContext.getSessionManager().removeClientSession(provider);
                    logger.error("[provider:{}] lost connection, removed from ClientSession", provider);
                    continue;
                }
            } else if (providerClientList.size() > 1) {
                logger.error("[providerClientList:{}] are multiple duplicate [RegisterVO:{}]", providerClientList, providerCache);
                continue;
            }
            ...
            if (Objects.isNull(session)) {
                ...
                // 连接上了服务提供者
                session.setProviderAttribute(providerCache);
                EventBus.publicEvent(ConsumerStartEvent.valueOf(providerCache, session));
