...
public class SessionUtils {
    ...
    public static Session getSession(ChannelHandlerContext ctx) {
        Attribute<Session> sessionAttr = ctx.channel().attr(Session.SESSION_KEY);
    ...
    public static String sessionInfo(Session session) {
        ...
        return StringUtils.format(CHANNEL_INFO_TEMPLATE, remoteAddress, session.getSessionId(), session.getUserId());
    }
    ...
    public static String sessionSimpleInfo(Session session) {
        ...
        return StringUtils.format(CHANNEL_SIMPLE_INFO_TEMPLATE, session.getSessionId(), session.getUserId());
    }

    public static Session initChannel(Channel channel) {
        Attribute<Session> sessionAttr = channel.attr(Session.SESSION_KEY);
            ...
            while (true) {
                ...
                NetContext.getSessionManager().forEachClientSession(new Consumer<Session>() {
                    ...
                    public void accept(Session session) {
                        builder.append(StringUtils.format("[session:{}，sid:{}]", session.getChannel().remoteAddress(),session.getSessionId()));
                        builder.append(FileUtils.LS);
                    }
                });
