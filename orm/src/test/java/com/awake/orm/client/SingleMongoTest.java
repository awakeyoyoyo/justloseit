...
//            .credential(MongoCredential.createCredential("root", "admin", "123456".toCharArray()))
    ...
    public void insertManyTest() {
        ...
        for (int i = 0; i < 10; i++) {
            var studentName = "Rashford" + i;
            var document = new Document("name", studentName).append("email", studentName + "@awake.com").append("age", i);
            datas.add(document);
        }
        collection.insertMany(datas);
        // 查询年龄大于等于20，小于25 $gte 运算符用于匹配字段大于等于（>=）指定值的文档
